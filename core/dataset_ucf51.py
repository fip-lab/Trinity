...
__all__ = ['UCF51Dataset', 'UCF51DataLoader']


class UCF51Dataset(Dataset):
    def __init__(self, args, mode='train'):
        ...
        data = data[mode]
        self.ids = np.array(data['FilePath'])
        self.cls_names = np.array(data['class_name'])
...
def UCF51DataLoader(args, mode='train'):
    if mode == 'train':
        datasets = {
            'train': UCF51Dataset(args, 'train'),
            # 'valid': UCF51Dataset(args, 'valid'),
            'test': UCF51Dataset(args, 'test')
        }
    else:
        datasets = {
            'test': UCF51Dataset(args, 'test')
